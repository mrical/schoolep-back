  ...
  return (
    <Row gutter={[12, 0]} key={item.id}>
      ...
      <Col className="gutter-row" span={4}>
        ...
        >
          {moneyFormatter({ amount: item.price.unit_amount })}
        </p>
      </Col>
      <Col className="gutter-row" span={4}>
        <p
...
export default function ReadItem({ config, selectedItem }) {
  ...
  useEffect(() => {
    ...
    if (currentResult) {
      const { lines, invoice, ...others } = currentResult;
      ...
      if (lines.data.length > 0) {
        setItemsList(lines.data);
        setCurrentErp(currentResult);
      } else if (invoice?.items) {
        setItemsList(invoice.lines);
        setCurrentErp({ ...invoice.lines, ...others, ...invoice });
      }
    }
    ...
    <>
      <PageHeader
        ...
        extra={[
          ...
          <Button
            key={`${uniqueId()}`}
            onClick={() => {
              window.open(currentErp.invoice_pdf);
            }}
      ...
      >
        <Row>
          ...
          <Statistic
            title="Balance"
            value={moneyFormatter({ amount: currentErp.amount_due })}
            style={{
              margin: '0 32px',
            }}
          />
      ...
      <Divider dashed />
      <Descriptions title={`Client : ${currentErp.customer_name}`}>
        <Descriptions.Item label="Address">{currentErp.customer_address?.city}</Descriptions.Item>
        <Descriptions.Item label="E-mail">{currentErp.customer_email}</Descriptions.Item>
        <Descriptions.Item label="Phone">{currentErp.customer_phone}</Descriptions.Item>
      </Descriptions>
      <Divider />
      <Row gutter={[12, 0]}>
        <Col className="gutter-row" span={11}>
      ...
      >
        <Row gutter={[12, -5]}>
          ...
          <Col className="gutter-row" span={12}>
            <p>Tax Total ({currentErp.default_tax_rates[0] * 100} %) :</p>
          </Col>
          <Col className="gutter-row" span={12}>
            <p>{moneyFormatter({ amount: currentErp.tax })}</p>
          </Col>
          <Col className="gutter-row" span={12}>
            <p>Total :</p>
          </Col>
